...
public class CommunityController {
    ...
    @Operation(summary = "게시물 댓글 삭제 API", description = "정보/질문 게시판에서 내가 작성한 댓글을 삭제하는 API입니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "게시물 댓글 삭제 성공", content = { @Content(mediaType = "application/json", schema = @Schema(implementation = Message.class) ) } ),
            @ApiResponse(responseCode = "400", description = "게시글 댓글 삭제 실패", content = { @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class) ) } ),
    })
    @DeleteMapping("/{postId}/{commentId}")
    public ResponseEntity<?> deletePostComment(
            @Parameter(description = "Access Token을 입력해주세요.", required = true) @CurrentUser UserPrincipal userPrincipal,
            @Parameter(description = "게시글 아이디를 입력해주세요.", required = true) @PathVariable Long postId,
            @Parameter(description = "댓글 아이디를 입력해주세요.", required = true) @PathVariable Long commentId
    ) {
        return communityService.deletePostComment(userPrincipal, postId, commentId);
    }
